...
for (let tick = 0; tick < 20; tick += 1) {
  market = tickMarket(market, tick * 2_000, pressures, rng);
}

const snapshot = toMarketSnapshot(market, 40_000);
for (const asset of snapshot.assets) {
  const sign = asset.changePct >= 0 ? '+' : '';
  console.log(`${asset.symbol.padEnd(5)} $${asset.price.toFixed(asset.kind === 'crypto' ? 4 : 2)}  ${sign}${asset.changePct.toFixed(2)}%`);
